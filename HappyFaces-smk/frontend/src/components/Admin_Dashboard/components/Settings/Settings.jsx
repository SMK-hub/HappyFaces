...
const Settings = () => {
  ...
  useEffect(() => {
    const fetchPhoto = async () => {
      try {
        const response = await axios.get(`http://localhost:8079/admin/getPhoto/${userDetails?.donorId}`, {
  ...
  const handleImageChange = async (file) => {
    ...
    if (file) {
      ...
      try{
        const response=await axios.post(`http://localhost:8079/admin/addPhoto/${userDetails?.donorId}`,formData);
        const status=response.status;
        console.log(status);
        if(status ===200){
          setUserData(response.data);

...
const SignInAdmin = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.post("http://localhost:8079/admin/login", orphanageDetails);
      const status = response.status;
      console.log(status);
      if (status === 200) {
        const userDetailResponse = await axios.get(`http://localhost:8079/admin/admin/${orphanageDetails.email}`);
  ...
  const fetchOtp = async()=>{
    try{
      const response = await axios.post(`http://localhost:8079/admin/sendOtp`,forgotPasswordData);
      const status=response.status;
      if(status === 200){
  ...
  const changePassword = async()=>{
    try{
        const response = await axios.post(`http://localhost:8079/admin/ForgetPassword/${forgotPasswordData.email}/${enteredOtp}/${newPasswordData.password}/${newPasswordData.confirmPassword}`);
        const status = response.status;
        if(status === 200){
          alert(response.data);
  ...
  return (
    <div className='sign-in-orphanage'>
      ...
      <div className="sign-in-orphanage-container">
        <h2 className="sign-in-orphanage-heading">Admin Sign In</h2>
